...
class GFP_Stripe {
	 ...
	 * @var string
	 */
	private static $version = '1.9.2.14';
	 ...
	 * @var string
	 */
	private static $min_gravityforms_stripe_more_version = '1.9.2.23';
	...
	public function admin_menu() {
		...
		if ( ( 'gf_edit_forms' == GFForms::get( 'page' ) ) && ( 'settings' == rgget( 'view' ) ) && ( 'stripe' == rgget( 'subview' ) || 'Stripe' == rgget( 'addon' ) ) && ( self::$_this->has_access( 'gfp_stripe_form_settings' ) ) ) {
	...
	public function admin_init() {
		...
		if ( in_array( RG_CURRENT_PAGE, array( 'admin-ajax.php' ) ) ) {
			...
			switch ( GFForms::get( 'page' ) ) {

				case 'gf_settings':

					GFForms::add_settings_page( 'Stripe', array( self::$_this, 'settings_page' ) );
	...
	function gform_noconflict_scripts( $noconflict_scripts ) {
		...
		if ( ( 'gf_settings' == GFForms::get( 'page' ) ) && ( 'Stripe' == rgget( 'subview' ) ) ) {
			$noconflict_scripts = array_merge( $noconflict_scripts, array( 'gfp_stripe_settings_page_js' ) );
		} else if ( 'gf_edit_forms' == GFForms::get( 'page' ) ) {
	 ...
	 *
	 * GFForms::print_scripts calls wp_print_scripts, causing default WordPress JS to break (such as contextual help
	...
	function wp_loaded() {
		if ( ( 'gf_edit_forms' == GFForms::get( 'page' ) ) && ( 'settings' == rgget( 'view' ) ) && ( 'stripe' == rgget( 'subview' ) || 'Stripe' == rgget( 'addon' ) ) && ( self::$_this->has_access( 'gfp_stripe_form_settings' ) ) ) {
			remove_action( 'admin_print_scripts', array( 'GFForms', 'print_scripts' ) );
				...
				<tr valign="top">
					...
					<td>
						...
						if ( class_exists( 'GFPMoreStripeUpgrade' ) ) {
							$version_info = GFPMoreStripeUpgrade::get_version_info( GFPMoreStripe::get_slug(), $gfp_support_key, GFPMoreStripe::get_version(), ( empty( $settings[ 'enable_early_access' ] ) ? false : $settings[ 'enable_early_access' ] ), ( isset( $_POST[ "gfp_stripe_submit" ] ) ? false : true ) );
		...
		foreach ( $valid_keys as $key => $value ) {
			...
			if ( ! empty( $settings[ $key ] ) ) {
				...
				try {
					...
					if ( false !== strpos( $key, 'secret' ) ) {

						PPP\Stripe\Account::retrieve();
					...
					if ( 'PPP\Stripe\Exception\CardException' == $class ) {
